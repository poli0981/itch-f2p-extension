...
import {EDITABLE_FIELDS, GENRE_PRESETS, MSG} from "../shared/constants.js";
...
function createCard (game) {
    ...
    if (game.platforms && game.platforms !== "N/A") meta.appendChild (makeMetaTag (truncate (game.platforms, 20)));
    ...
    // Tags (comma-separated string → chips)
    if (game.tags && game.tags !== "N/A") {
        const tagList = game.tags.split (",")
                            .map ((t) => t.trim ())
                            .filter (Boolean);
        if (tagList.length > 0) {
...
function createGenreField (game) {
    ...
    // Build options from detected tags + presets
    const detectedTags = game.tags && game.tags !== "N/A"
                         ? game.tags.split (",")
                               .map ((t) => t.trim ())
                               .filter (Boolean)
                         : [];
